...
/* Details */
.row-details{display:none;padding:4px 14px 22px 46px;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:10px;}
  ...
  <div class="brand-header">
    <div class="brand-left">
      ...
      <div>
        ...
        <div class="brand-sub">Quality Check System · Auto Quotes</div>
      </div>
    </div>
...
async function handleFiles(files){
  ...
  for(let i=0;i<pdfs.length;i++){
    ...
    try{
      ...
      allResults.push(analyzeQuote(text,pdfs[i].name));
    }catch(e){
...
function analyzeQuote(text,filename){
 ...
 // ── Dates — exactly 7 days from Prepared on ──
  ...
  if(prepMatch&&startMatch){
    ...
    if(pd&&sd){
      ...
      if(!dateOk){
        ...
        else errors.push(`Policy start date too far out (${dateDiff} days — must be exactly 7)`);
      }
    }
  ...
  // ── Home Valuation (bundled Home/Property quotes) ──
  const homeCheck=checkHomeValuation(t,errors);
  ...
  return{filename,name,quoteType,monthlyEFT,vehicleCount,vehicles,drivers,
    errors,warnings,status,putInStop,
    checks:{dateOk,dateDiff,prepDateStr,startDateStr,premiumOk,...homeCheck}};
}
...
function extractVehicles(t){
  ...
  // isVal: accepts $1,000 / $500 / $250 / - / Included / None
  const isVal=v=>/^\$[\d,]+$/.test(v)||/^-$/.test(v)||/^included$/i.test(v)||/^none$/i.test(v);
  // toNum: returns string like '1000', '500', or null for - / Included / None
  const toNum=v=>{
  ...
  // Find "Coverage for your property" — scan all occurrences, use the one with vehicle lines
  let tableStart=-1;
  ...
  // Collect raw entries from table (skip headers/noise)
  const entries=[];
  for(let i=tableStart+1;i<lines.length&&i<tableStart+100;i++){
    ...
    // Skip lines that are just column headers repeated
    if(/^Comp(rehensive)?\s+Coll(ision)?$/i.test(line)) continue;
    entries.push(line);
  }

  // --- Strategy 1: Try interleaved layout first ---
  // Pattern: VehName [optional val1] [optional val2], repeat
  const tempVehicles=[];
  let i=0;
  while(i<entries.length){
    const line=entries[i];
    const m=line.match(/^((?:19|20)\d{2})\s+([A-Za-z0-9][a-zA-Z0-9 \-\/\.]{2,50})/);
    if(m){
      const year=parseInt(m[1]);
      const model=m[2].trim();
      let compVal=null,collVal=null;
      // Check inline values on same line first (e.g. "2019 Honda Civic $1,000 $1,000")
      const inlineVals=line.match(/(\$[\d,]+|-)\s+(\$[\d,]+|-)$/);
      if(inlineVals){
        compVal=toNum(inlineVals[1]);
        collVal=toNum(inlineVals[2]);
      } else {
        // Look ahead for values on next lines
        const next1=entries[i+1]||'';
        const next2=entries[i+2]||'';
        if(isVal(next1)){
          compVal=toNum(next1);
          if(isVal(next2)) collVal=toNum(next2);
        }
      }
  ...
  // --- Strategy 2: Detect grouped layout ---
  // Grouped = all vehicles have null comp AND null coll after strategy 1
  const allNull=tempVehicles.length>0&&tempVehicles.every(v=>v.compVal===null&&v.collVal===null);
  if(allNull){
    // Collect all value tokens in document order after the last vehicle name entry
    const lastVehIdx=Math.max(...tempVehicles.map(v=>v._entryIndex));
    const valBlock=[];
    for(let j=0;j<entries.length;j++){
      // Include value entries that appear after the first vehicle or are between vehicle lines
      if(isVal(entries[j])) valBlock.push(entries[j]);
    }
    // Assign pairs: comp then coll per vehicle
    for(let j=0;j<tempVehicles.length;j++){
      tempVehicles[j].compVal=valBlock[j*2]!==undefined?toNum(valBlock[j*2]):null;
      tempVehicles[j].collVal=valBlock[j*2+1]!==undefined?toNum(valBlock[j*2+1]):null;
    }
  }

  // Clean up internal property and push
...
// ── HOME / PROPERTY VALUATION (bundled Farmers Home quotes) ──
// Only applies when a bundled PDF includes a Home/Property quote summary section.
function checkHomeValuation(t,errors){
  const ppvMatch=t.match(/Personal\s+property\s+valuation\s+([A-Za-z][A-Za-z\s]*?)(?=\n|$)/i);
  const roofMatch=t.match(/Roof\s+valuation\s+([A-Za-z][A-Za-z\s]*?)(?=\n|$)/i);
  const hasHome=!!(ppvMatch||roofMatch);
  let ppvOk=true,roofOk=true,ppvVal=null,roofVal=null;
  if(ppvMatch){
    ppvVal=ppvMatch[1].trim();
    ppvOk=/replacement\s+cost/i.test(ppvVal);
    if(!ppvOk) errors.push(`Personal Property Valuation must be Replacement Cost (found "${ppvVal}")`);
  }
  if(roofMatch){
    roofVal=roofMatch[1].trim();
    roofOk=/replacement\s+cost/i.test(roofVal);
    if(!roofOk) errors.push(`Roof Valuation must be Replacement Cost (found "${roofVal}")`);
  }
  return{hasHome,ppvOk,roofOk,ppvVal,roofVal};
}

function checkLiability(t,errors){
  // Layout is a header row followed by a value row, e.g.:
  // "Bodily injury liability Property damage" \n "$25,000/$50,000 $25,000"
  const m=t.match(/Bodily\s+injury[\s\S]{0,60}?Property\s+damage[\s\S]{0,15}?\$?([\d,]+)\s*\/\s*\$?([\d,]+)\s+\$?([\d,]+)/i);
  if(m){
    const bi1=m[1].replace(/,/g,''),bi2=m[2].replace(/,/g,''),pd=m[3].replace(/,/g,'');
    if(bi1!=='25000'||bi2!=='50000') errors.push('Bodily Injury must be $25,000/$50,000');
    if(pd!=='25000') errors.push('Property Damage must be $25,000');
  } else {
    errors.push('Bodily Injury must be $25,000/$50,000');
    errors.push('Property Damage must be $25,000');
  }
}
// ── TYPE 1: FARMERS ──
function checkFarmers(t,errors,warnings,vehicles){
  checkLiability(t,errors);
...
function checkPureBristol(t,errors,warnings,vehicles){
  ...
  const discMatch=t.match(/Discounts\s*\/\s*Preferences\s+([\s\S]{0,350}?)(?:Monthly\s+premium|Applied\s+discounts|Payment\s+plan)/i);
...
function buildDetails(r){
  ...
  // Home valuation section (only if bundle has a Home/Property quote)
  const homeHTML=c.hasHome?`
    <div class="detail-group">
      <h4>Property Valuation</h4>
      ${ck(c.ppvOk,'Personal Property Valuation',c.ppvVal||'')}
      ${ck(c.roofOk,'Roof Valuation',c.roofVal||'')}
    </div>`:'';

  return `
      <div class="detail-group">
      <h4>Liability</h4>
      ${ck(!r.errors.some(e=>/Bodily Injury must be/i.test(e)),'Bodily Injury','$25,000/$50,000')}
      ${ck(!r.errors.some(e=>/Property Damage must be/i.test(e)),'Property Damage','$25,000')}
    </div>
    ${homeHTML}
    <div class="detail-group">
